...
module top_r2b_circular_fifo #(
    ...
    logic [RD_DATA_COUNT_WIDTH-1:0] rd_data_count_sig [TOTAL_INPUT_W_Qn_KnT][NUM_BANKS_FIFO];
    ...
    always_comb begin
        for (int a = 0; a < NUM_BANKS_FIFO; a++) begin
            ...
            rd_data_count[a]    = rd_data_count_sig[0][a];
        ...
        for (j = 0; j < TOTAL_INPUT_W_Qn_KnT; j++) begin : GEN_FIFO_BANK
            for (i = 0; i < NUM_BANKS_FIFO; i++) begin
                xpm_fifo_sync #(
                    ...
                    .rd_data_count(rd_data_count_sig[j][i]),       // Updated
