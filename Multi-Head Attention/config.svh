...
//	`define SYSTEM_TOP_WIDTH_KEYS (`SYSTEM_TOP_WIDTH_INPUT + 4)
//`endif

//`ifndef SYSTEM_FRAC_WIDTH_KEYS
//	`define SYSTEM_FRAC_WIDTH_KEYS (`SYSTEM_FRAC_WIDTH_INPUT + 2)
//`endif
`ifndef SYSTEM_TOP_WIDTH_KEYS
	`define SYSTEM_TOP_WIDTH_KEYS (`SYSTEM_TOP_WIDTH_INPUT + 0)
`endif

`ifndef SYSTEM_FRAC_WIDTH_KEYS
	`define SYSTEM_FRAC_WIDTH_KEYS (`SYSTEM_FRAC_WIDTH_INPUT + 0)
`endif

// Q_KT width
`ifndef SYSTEM_TOP_WIDTH_QKT
	`define SYSTEM_TOP_WIDTH_QKT (`SYSTEM_TOP_WIDTH_KEYS + 0)
`endif

`ifndef SYSTEM_FRAC_WIDTH_QKT
	`define SYSTEM_FRAC_WIDTH_QKT (`SYSTEM_FRAC_WIDTH_KEYS + 0)
`endif
//`ifndef SYSTEM_TOP_WIDTH_QKT
